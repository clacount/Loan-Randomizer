...
If an initial run returns REVIEW, LendingFair performs additional assignment attempts and selects the best available assignment. If the selected result still returns REVIEW, approver confirmation is required before running totals/history and reports are saved.
